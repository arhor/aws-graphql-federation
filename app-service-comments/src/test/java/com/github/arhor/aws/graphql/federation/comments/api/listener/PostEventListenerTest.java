...
class PostEventListenerTest extends EventListenerTestBase {
    ...
    static void registerDynamicProperties(final DynamicPropertyRegistry registry) {
        registry.add("app-props.events.source.sync-comments-on-post-created-event", () -> POST_CREATED_TEST_QUEUE);
        registry.add("app-props.events.source.sync-comments-on-post-deleted-event", () -> POST_DELETED_TEST_QUEUE);
    }
        ...
        await()
            ...
            .untilAsserted(() -> {
                then(postRepresentationService)
                    .should()
                    .createPostRepresentation(event.getId(), event.getUserId(), TRACE_ID);
    ...
    void should_call_deletePostRepresentation_method_on_post_deleted_event() {
        ...
        await()
            ...
            .untilAsserted(() -> {
                then(postRepresentationService)
                    .should()
                    .deletePostRepresentation(event.getId(), TRACE_ID);
