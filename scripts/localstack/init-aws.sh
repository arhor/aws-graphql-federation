...
user_updates_for_comments_url=$(awslocal sqs create-queue --queue-name user-updates-for-comments --output text)
user_deletes_for_comments_url=$(awslocal sqs create-queue --queue-name user-deletes-for-comments --output text)
...
user_updates_for_comments_arn=$(awslocal sqs get-queue-attributes --queue-url "$user_updates_for_comments_url" --attribute-names QueueArn --query Attributes --output text)
user_deletes_for_comments_arn=$(awslocal sqs get-queue-attributes --queue-url "$user_deletes_for_comments_url" --attribute-names QueueArn --query Attributes --output text)
...
awslocal sns subscribe \
    --topic-arn "$user_changes_arn" \
    --protocol sqs \
    --notification-endpoint "$user_updates_for_comments_arn" \
    --attributes '{ "FilterPolicy": "{\"x_payload_type\":[\"UserChange.Updated\"]}", "RawMessageDelivery": "true" }'

awslocal sns subscribe \
    --topic-arn "$user_changes_arn" \
    --protocol sqs \
    --notification-endpoint "$user_deletes_for_comments_arn" \
    --attributes '{ "FilterPolicy": "{\"x_payload_type\":[\"UserChange.Deleted\"]}", "RawMessageDelivery": "true" }'
